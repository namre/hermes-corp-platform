...
Один корпоративный ИИ-агент на всех не годится — данные людей смешаются. Поэтому
здесь **каждому сотруднику выдаётся свой изолированный контейнер-агент** с
собственной памятью, файлами и доступами к корпоративным системам. Память и
переписка одного человека не видны другому.

Работают через привычные каналы — **Telegram или веб (OpenWebUI)**. Что бы
сотрудник ни выбрал, запрос идёт в **identity-proxy**: он определяет, кто пишет, и
направляет в его персональный runtime. Поэтому сессии из разных источников — это
**один контекст**: написал утром в вебе, днём в Telegram — агент тот же и помнит
начатое.

Всё **self-hosted**: переписка, файлы и память остаются на вашем сервере и не
уезжают в чужое облако и чужое обучение.
